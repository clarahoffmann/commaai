...
class comp_dens():
    ...
    def compute_dens(self, method):
        ...
        if self.method == 'va_horseshoe':
            ...
            self.mu_t_va = np.load(self.va_horse_dir + 'mu_ts2_new_dev.npy')
            self.iteration = self.mu_t_va.shape[0]
            self.p = 10
            self.B_ts = np.mean(np.load(self.va_horse_dir + 'B_ts2_new_dev.npy')[int(0.9*self.iteration):,:,:], axis = 0)
            self.d_ts = np.mean(np.load(self.va_horse_dir + 'd_ts2_new_dev.npy')[int(0.9*self.iteration):,:,:], axis = 0)
        ...
        if self.method == 'hmc_horseshoe':
            self.hmc_thetas = np.load('../../../../data/commaai/mcmc/filtered_gaussian_resampled/Horseshoe/all_thetas_try.npy')
        ...
        if self.method == 'hmc_ridge':
            ...
            self.hmc_thetas = np.load(str(self.hmc_ridge_dir + 'all_thetas_L100_3000.npy'))[1000:, :]
