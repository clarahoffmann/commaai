...
class comp_dens():
    ...
    def compute_dens(self, method):
        ...
        if self.method == 'va_horseshoe':
            ...
            self.mu_t_va = np.load(self.va_horse_dir + 'mu_ts_new.npy')
            self.iteration = self.mu_t_va.shape[0]
            self.p = 10
            self.B_ts = np.mean(np.load(self.va_horse_dir + 'B_ts_new.npy')[int(0.95*self.iteration):,:,:], axis = 0)
            self.d_ts = np.mean(np.load(self.va_horse_dir + 'd_ts_new.npy')[int(0.95*self.iteration):,:,:], axis = 0)
        ...
        if self.method == 'hmc_horseshoe':
            self.hmc_thetas = np.load('../../../../data/commaai/mcmc/filtered_gaussian_resampled/Horseshoe/all_thetas.npy')
            self.hmc_thetas[:,10:20] = np.exp(0.5*self.hmc_thetas[15000:,10:20])
            self.hmc_thetas[:,20] = np.exp(self.hmc_thetas[15000:,20])
        ...
        if self.method == 'va_ridge':
            ...
            self.iteration = self.mu_t_va.shape[0]
            self.beta = np.mean(self.mu_t_va[int(0.95*self.iteration):self.iteration,0:10], axis = 0)
            self.tau = np.exp(np.mean(self.mu_t_va[int(0.95*self.iteration):self.iteration,10], axis = 0))
            self.p = len(self.beta)
            self.Lambdas = None
            
        if self.method == 'hmc_ridge':
            self.hmc_ridge_dir = '../../../../data/commaai/mcmc/filtered_gaussian_resampled/Ridge/'
            self.hmc_thetas = np.load(str(self.hmc_ridge_dir + 'all_thetas_new.npy'))[500:, :]
