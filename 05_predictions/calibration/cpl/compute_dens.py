...
class comp_dens():
    ...
    def compute_dens(self, method):
        ...
        if self.method == 'va_horseshoe':
            ...
            self.mu_t_va = np.load(self.va_horse_dir + 'mu_ts.npy')
            self.iteration = self.mu_t_va.shape[0]
            self.p = 10
            self.B_ts = np.mean(np.load(self.va_horse_dir + 'B_ts.npy')[int(0.95*self.iteration):,:,:], axis = 0)
            self.d_ts = np.mean(np.load(self.va_horse_dir + 'd_ts.npy')[int(0.95*self.iteration):,:,:], axis = 0)
        ...
        if self.method == 'hmc_ridge':
            ...
            self.hmc_thetas = np.load(str(self.hmc_ridge_dir + 'all_thetas.npy'))[500:, :]
