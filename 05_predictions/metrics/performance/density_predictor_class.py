...
class density_predictor():
    ...
    def get_density(self, method):
        ...
        if self.method == 'va_ridge':
            ...
            self.tau_sq = np.exp(np.mean(self.mu_t_va[int(0.95*self.iterations):self.iterations,10], axis = 0))
        ...
        if self.method == 'va_horseshoe':
            ...
            self.tau_sq = np.exp(np.mean(self.mu_t_va[int(0.95*self.iterations):self.iterations,20], axis = 0))
