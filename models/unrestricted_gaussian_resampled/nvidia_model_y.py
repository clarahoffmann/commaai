...
model_dir = '../../../data/models/20201021_unrestr_gaussian_resampled_y/'
...
#corrupted_files = []
#total_images = 0
#for f_i, file in enumerate(shard_files): 
#    try: 
#        total_images += sum([1 for _ in tf.compat.v1.io.tf_record_iterator(file)])
#    except:
#        print('error at ' + str(f_i))
#        corrupted_files.append(f_i)
#train_files = train_files.remove(corrupted_files)
...
def imgs_input_fn(filenames, perform_shuffle = True, repeat_count = EPOCHS, batch_size = 16): 
...
config = tf.estimator.RunConfig(log_step_count_steps=5, save_summary_steps = 5))
...
eval_spec = tf.estimator.EvalSpec(input_fn = cust_val_input_fn, 
                                  steps = None, start_delay_secs=120, throttle_secs=60) # , exporters = exporter for exporter, receiver must also be defined
