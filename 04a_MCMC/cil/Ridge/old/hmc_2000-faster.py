...
def log_density(z, u,  beta, B, p,  n, S, S2, tBB, theta, betaBt):   
    return (- 0.5*np.sum(np.log(S2)) - 0.5*z.dot((1/S2)*z) + betaBt.dot(z*(1/S))- 0.5*beta.T.dot(tBB).dot(beta) - 0.5/np.exp(u)*np.sum(beta**2)- 0.5*u*(p-1)  - np.sqrt(np.exp(u)/theta))
...
for m in tqdm(range(1, M - 1)):
    ...
    if np.random.randn() <= alpha[m]:
        ...
        all_thetas.append(np.array(theta_m_1[m + 1]))
    if ((m % 50 == 0) & (m > 1)):
        print(np.mean(acc[(m - 50):]))
        np.save('../../../../data/commaai/mcmc/unfiltered_gaussian_resampled/Ridge/all_thetas_L2000_F.npy', np.array(all_thetas))
    #if m < tune:
...
np.save('../../../../data/commaai/mcmc/filtered_gaussian_resampled/Ridge/all_thetas_L2000_F.npy', np.array(all_thetas))