...
def generate_dS2_ddS2_S2_S(Lambda, BoB):
    ...
    #ddS2 = ( - (BoB.dot(np.diag(Lambda2))) +  (BoB.dot(np.diag(Lambda2))))/(((1+np.tile(W, [p,1]))**3).T)
...
def log_density(S, B, beta, Lambda, log_tau, z, p):
    ...
    S2 = S**2
    return( - 0.5*np.sum(np.log(S2)) + 0.5*np.sum(Lambda2) -0.5*np.sum(((z - S*(B.dot(beta)))**2)*(1/S2)) 
           -0.5*np.sum((beta**2)/(Lambda2)) -(p-1)*log_tau - np.sum(np.log(1+Lambda2/tau2)) - np.log(1 + tau2) )
...
for m in tqdm(range(1, M)):
    ...
    if np.random.randn() <= alpha[m]:
        ...
        all_thetas.append(np.array(theta_m_1[m + 1]))
    if (m % 50 == 0) & (m > 1):
        print(np.mean(acc[-50:]))
        np.save('../../../../data/commaai/mcmc/filtered_gaussian_resampled/Horseshoe/all_thetas_L100.npy', np.array(all_thetas))
        
np.save('../../../../data/commaai/mcmc/filtered_gaussian_resampled/Horseshoe/all_thetas_L10002.npy', np.array(all_thetas))