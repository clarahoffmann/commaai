...
def Fy(y, density, density_type = 'fast_kde' ):
    '''Computes the cdf of the steering wheel angles at value y
      based on the Shimazaki and S. Shinomoto density 

      Inputs:
      y : value at which the CDF should be evaluated
      density : dataframe with densities obtained with the Shimazaki and 
                S. Shinomoto with columns 'time' and 'dens', which contain
                the values at which the density is evaluated and the density
                themselves respectively

      Output:
      integral: Pr(Y <= y) = CDF of Y at value y
    '''
    if density_type == 'fast_kde':
        function_values = density['pdf'][(density['y'] <= y)]
        x_values = density['y'][(density['y'] <= y)]
        integral = simps(function_values, x_values)
    
    elif density_type == 'fast_kde':
        function_values = density.dens[(density.time <= y)]
        x_values = density.time[(density.time <= y)]
        integral = simps(function_values, x_values)
